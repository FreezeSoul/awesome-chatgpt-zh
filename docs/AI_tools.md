## AI 生产力工具

> 2024–2026 更新：覆盖中外主流工具，含大量国产代表。

### AI 对话助手

- [ChatGPT](https://chat.openai.com) — OpenAI 推出的通用 AI 对话助手，生成式 AI 浪潮的引爆者。
- [Claude](https://claude.ai) — Anthropic 出品，长文本理解、写作与代码能力出众的安全型助手。
- [Gemini](https://gemini.google.com) — Google 的多模态大模型，深度集成谷歌全家桶。
- [Grok](https://grok.com) — xAI（马斯克）推出，强调实时信息与个性化的对话助手。
- [DeepSeek](https://www.deepseek.com) — 国产开源大模型代表，V4 系列推理、数学与编码能力比肩顶级闭源模型，网页与 App 免费；详见 [DeepSeek 生态指南](DeepSeek.md)。
- [Kimi](https://kimi.moonshot.cn) — 月之暗面出品，擅长超长文本处理与深度推理的国产助手。
- [豆包](https://www.doubao.com) — 字节跳动推出的国产 AI 助手，用户规模领先，多模态全能。
- [通义千问](https://tongyi.aliyun.com) — 阿里云的大模型助手，支持多轮对话、多模态与多语言。
- [文心一言](https://yiyan.baidu.com) — 百度基于文心大模型的中文对话与创作助手。

### AI 搜索

- [Perplexity](https://www.perplexity.ai) — 答案直给且附带引用来源的 AI 搜索引擎，研究与事实核查利器。
- [秘塔 AI 搜索](https://metaso.cn) — 国产无广告 AI 搜索，结果结构化、可生成大纲与脑图。
- [Genspark](https://www.genspark.ai) — AI Agent 搜索引擎，可自动生成专题页。
- [天工 AI 搜索](https://www.tiangong.cn) — 昆仑万维出品，无广告结果的国产 AI 搜索。
- [LLocalSearch](https://github.com/nilsherzig/LLocalSearch) — 完全本地运行的开源 AI 搜索聚合器，用 LLM 智能体链回答问题并展示推理进度，无需 OpenAI/Google API key。

### AI 绘画/图像

- [Midjourney](https://www.midjourney.com) — 艺术表现力顶尖的 AI 绘画工具，画面质感与审美行业标杆。
- [DALL·E 3](https://openai.com/dall-e-3) — OpenAI 图像模型，集成于 ChatGPT，擅长理解复杂指令。
- [Stable Diffusion](https://stability.ai) — Stability AI 的开源图像生成模型，可本地部署、生态丰富。
- [FLUX](https://blackforestlabs.ai) — Black Forest Labs 的新一代开源图像模型，画质媲美顶级商用产品。
- [Ideogram](https://ideogram.ai) — 擅长在图像中精准生成文字与排版的 AI 绘画工具。
- [Adobe Firefly](https://www.adobe.com/products/firefly.html) — Adobe 的创意生成式 AI，版权安全、深度集成创意套件。
- [即梦 Dreamina](https://jimeng.jianying.com) — 字节跳动剪映团队的国产 AI 图像与视频创作平台。
- [文心一格](https://yige.baidu.com) — 百度的 AI 艺术创作平台，支持多种风格、对新手友好。
- [remove-ai-watermarks](https://github.com/wiltodelta/remove-ai-watermarks) — 开源 CLI / Python 库，用于去除图片中的可见与隐形 AI 水印（如 Gemini/Nano Banana sparkle、SynthID）及来源元数据（C2PA、EXIF、IPTC）。

### AI 视频生成

- [Sora](https://openai.com/sora) — OpenAI 的文生视频模型，物理一致性与画面真实感行业瞩目。
- [Runway](https://runwayml.com) — 面向创作者的 AI 视频平台，强在镜头控制与编辑工作流。
- [Pika](https://pika.art) — 主打性价比的 AI 视频生成工具，上手简单、风格化效果好。
- [Luma Dream Machine](https://lumalabs.ai/dream-machine) — Luma 出品，画面电影感强、镜头运动自然。
- [可灵 Kling](https://klingai.kuaishou.com) — 快手国产视频大模型，人物动作真实、可生成长时长视频。
- [Vidu](https://www.vidu.cn) — 生数科技与清华联合出品的国产文生视频模型。
- [海螺 Hailuo](https://hailuoai.com/video) — MiniMax 出品的国产视频生成工具，画面流畅、效果稳定。

### AI 编程/IDE

- [Cursor](https://www.cursor.com) — 当下最热门的 AI 代码编辑器，代码库感知与多文件改写能力强。
- [GitHub Copilot](https://github.com/features/copilot) — GitHub 与 OpenAI 联手打造，最广泛使用的 AI 编程助手。
- [Windsurf](https://windsurf.com) — 原 Codeium 团队推出的 AI 原生代码编辑器，主打 Agent 能力。
- [Cline](https://cline.bot) — 开源自主编程 Agent，以 VS Code 插件形式运行、可接入任意大模型。
- [Trae](https://www.trae.ai) — 字节跳动推出的国产 AI IDE，对标 Cursor、对中文开发者友好。
- [通义灵码](https://lingma.aliyun.com) — 阿里云基于通义大模型的智能编码助手。
- [v0](https://v0.dev) — Vercel 出品，通过对话生成 React/Next.js 前端 UI 组件。
- [bolt.new](https://bolt.new) — StackBlitz 出品的全栈 AI 开发平台，从提示到可运行 Demo 极快。
- [Grok Build](https://github.com/xai-org/grok-build) — xAI 开源的终端编码 Agent，全屏 TUI，Apache-2.0。
- [OpenWorker](https://github.com/andrewyng/openworker) — 吴恩达开源的桌面 AI 同事，交付成品而非聊天，模型无关、本机运行。

### AI 写作

- [Notion AI](https://www.notion.so/product/ai) — 内嵌于 Notion 工作区的 AI 写作层，与笔记和文档无缝协作。
- [Jasper](https://www.jasper.ai) — 面向营销团队的 AI 写作平台，主打品牌语气与规模化产出。
- [Grammarly](https://www.grammarly.com) — 跨平台英文写作助手，专注语法纠错与表达润色。
- [copy.ai](https://www.copy.ai) — 擅长批量生成营销文案、邮件与广告变体的 AI 写作工具。
- [秘塔写作猫](https://xiezuocat.com) — 国产中文写作助手，主打纠错、改写与全文润色。

### AI PPT/设计

- [Gamma](https://gamma.app) — 当红 AI 演示工具，一句话生成 PPT、网页与文档。
- [beautiful.ai](https://www.beautiful.ai) — 智能模板自动排版的 AI 演示工具。
- [美图设计室](https://www.designkit.cn) — 美图出品的国产 AI 设计平台，主打商品图、抠图与海报。
- [Canva AI](https://www.canva.com) — Canva 的 Magic 系列 AI，把生成式设计带入海量模板。
- [稿定 AI](https://www.gaoding.art) — 国产一站式 AI 设计平台，覆盖文生图、文案与商品图。
- [GenOffice](https://github.com/genspark-ai/genoffice) — Genspark 开源的全功能 AI Office 套件（Word / Excel / PPT，macOS / Windows / Linux，Apache-2.0）。
- [OpenKnowledge](https://github.com/inkeep/open-knowledge) — Inkeep 开源的 AI 原生 Markdown IDE / LLM wiki：所见即所得编辑、图谱、与 Claude / Codex / OpenCode 并排协作，自带 MCP 与 Skills。

### AI 语音/音乐/数字人

- [ElevenLabs](https://elevenlabs.io) — 业界领先的 AI 语音生成与克隆平台，支持 70 多种语言。
- [Suno](https://suno.com) — 一句话生成完整歌曲（含人声）的 AI 音乐创作工具。
- [Udio](https://www.udio.com) — 主打高保真音质的 AI 音乐生成平台。
- [剪映](https://www.capcut.cn) — 字节出品的国产视频剪辑工具，内置 AI 配音、字幕与特效。
- [HeyGen](https://www.heygen.com) — 选择数字人、输入脚本即可生成 AI 口播视频。
- [Synthesia](https://www.synthesia.io) — 企业级 AI 数字人视频平台。

### AI 会议/效率

- [flowith](https://flowith.io) — 基于二维画布的 AI 工作空间，可同时与多个模型协作深度工作。
- [Napkin AI](https://www.napkin.ai) — 把文字一键转为信息图与可视化图表的 AI 工具。
- [Otter.ai](https://otter.ai) — 实时会议转录与摘要 AI，自动生成纪要、洞察与行动项。

### [AI 研究所](https://www.aiyjs.com/)

AI研究所：一个收录 AI 相关工具和AI资讯的中文网站

![ai_yjs](imgs/ai_yjs.jpg)